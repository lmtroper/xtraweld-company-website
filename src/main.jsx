...
function Header() {
  ...
  return <><section id="contact-header"><div><a href="tel:6478337344"><img src="media/phone-icon.png" alt="Phone" /><span>647-833-7344</span></a></div><div><a href="mailto:xtraweld@gmail.com"><img src="media/mail-icon.png" alt="Email" /><span>xtraweld@gmail.com</span></a></div></section>
...
function Contact() {
  ...
  const cards = [['contact-us-phone.png', 'Phone:', '647-833-7344'], ['contact-us-email.png', 'Email:', 'xtraweld@gmail.com'], ['contact-us-address.png', 'Address:', 'Brampton, Ontario']];
  return <><section id="contact-anchor" className="contact-form-section"><div className="filler-image col-xl-5 col-lg-5"><img src="media/contact_us2.jpg" alt="Welding work" /></div><div className="contact-form col-xl-8 col-lg-8 col-12"><div className="contact-form-title"><span className="subheader-bar" /><div>Contact Us</div></div><div className="contact-details">{cards.map(([image, label, value]) => <div className="contact-detail" key={label}><img src={`media/${image}`} alt="" /><span><b>{label}</b>{label === 'Email:' ? <a href="mailto:xtraweld@gmail.com">{value}</a> : label === 'Phone:' ? <a href="tel:6478337344">{value}</a> : <span>{value}</span>}</span></div>)}</div><div className="contact-form-thanks">Thank you for your interest in Xtra Weld services. You can contact us via email or by completing the form below.</div><div className="input-group"><form onSubmit={submit}>{[['name-input', 'Name', 'text'], ['company-input', 'Company', 'text'], ['email-input', 'Email', 'email'], ['phone-input', 'Phone Number', 'text']].map(([id, label, type]) => <div className="form-group" key={id}><label className={label === 'Name' || label === 'Email' ? 'required' : ''} htmlFor={id}>{label}</label><input type={type} className="form-control" id={id} name={id} placeholder={label} required={label === 'Name' || label === 'Email'} /></div>)}<div className="form-group"><label className="required" htmlFor="message-input">Message</label><textarea className="form-control" id="message-input" name="message" rows="5" placeholder="How can we help you?" required /></div><button id="submit-btn" type="submit" className="btn"><span>Send Message</span></button>{sent && <p role="status">Your message was sent successfully.</p>}</form></div></div></section></>;
}
